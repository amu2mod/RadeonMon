...
void ADLXGpuTelemetry::Init(HWND hwnd)
{
	...
	if (!version.valid)
	{
		...
		TaskDialog(m_hwnd, nullptr, L"ADLX Version Error", L"Unable to determine the ADLX version",
...
void ADLXGpuTelemetry::Discover()
{
	...
	discoverV0("GPU Usage", &IADLXGPUMetricsSupport::IsSupportedGPUUsage, &IADLXGPUMetricsSupport3::GetGPUUsageRange, GPU_CAP_USAGE, discoveredMetrics.usage);
	discoverV0("GPU Clock Speed", &IADLXGPUMetricsSupport::IsSupportedGPUClockSpeed, &IADLXGPUMetricsSupport3::GetGPUClockSpeedRange, GPU_CAP_CLOCK, discoveredMetrics.clockSpeed);
	discoverV0("VRAM Clock Speed", &IADLXGPUMetricsSupport::IsSupportedGPUVRAMClockSpeed, &IADLXGPUMetricsSupport3::GetGPUVRAMClockSpeedRange, GPU_CAP_VRAM_CLOCK, discoveredMetrics.vramClockSpeed);
	discoverV0("GPU Temperature", &IADLXGPUMetricsSupport::IsSupportedGPUTemperature, &IADLXGPUMetricsSupport3::GetGPUTemperatureRange, GPU_CAP_TEMP, discoveredMetrics.temperature);
	discoverV0("GPU Hotspot Temperature", &IADLXGPUMetricsSupport::IsSupportedGPUHotspotTemperature, &IADLXGPUMetricsSupport3::GetGPUHotspotTemperatureRange, GPU_CAP_HOTSPOT, discoveredMetrics.hotspot);
	discoverV0("GPU Power", &IADLXGPUMetricsSupport::IsSupportedGPUPower, &IADLXGPUMetricsSupport3::GetGPUPowerRange, GPU_CAP_POWER, discoveredMetrics.power);
	discoverV0("GPU Total Board Power", &IADLXGPUMetricsSupport::IsSupportedGPUTotalBoardPower, &IADLXGPUMetricsSupport3::GetGPUTotalBoardPowerRange, GPU_CAP_BOARD_POWER, discoveredMetrics.totalBoardPower);
	discoverV0("GPU Fan Speed", &IADLXGPUMetricsSupport::IsSupportedGPUFanSpeed, &IADLXGPUMetricsSupport3::GetGPUFanSpeedRange, GPU_CAP_FAN_SPEED, discoveredMetrics.fanSpeed);
	discoverV0("GPU VRAM", &IADLXGPUMetricsSupport::IsSupportedGPUVRAM, &IADLXGPUMetricsSupport3::GetGPUVRAMRange, GPU_CAP_VRAM_USAGE, discoveredMetrics.vram);
	discoverV0("GPU Voltage", &IADLXGPUMetricsSupport::IsSupportedGPUVoltage, &IADLXGPUMetricsSupport3::GetGPUVoltageRange, GPU_CAP_VOLTAGE, discoveredMetrics.voltage);
	discoverV0("GPU Intake Temperature", &IADLXGPUMetricsSupport::IsSupportedGPUIntakeTemperature, &IADLXGPUMetricsSupport3::GetGPUIntakeTemperatureRange, GPU_CAP_INTAKE_TEMP, discoveredMetrics.intakeTemperature);

	// ===== V1 =====

	discoverV1("GPU Memory Temperature", &IADLXGPUMetricsSupport1::IsSupportedGPUMemoryTemperature, &IADLXGPUMetricsSupport1::GetGPUMemoryTemperatureRange, GPU_CAP_MEM_TEMP, discoveredMetrics.memoryTemperature);
	...
	discoverV1("NPU Frequency", &IADLXGPUMetricsSupport1::IsSupportedNPUFrequency, &IADLXGPUMetricsSupport1::GetNPUFrequencyRange, GPU_CAP_NPU_FREQ, discoveredMetrics.npuFrequency);
	discoverV1("NPU Activity Level", &IADLXGPUMetricsSupport1::IsSupportedNPUActivityLevel, &IADLXGPUMetricsSupport1::GetNPUActivityLevelRange, GPU_CAP_NPU_ACTIVITY, discoveredMetrics.npuActivityLevel);

	// ===== V3 =====

	discoverV3("GPU Shared Memory", &IADLXGPUMetricsSupport3::IsSupportedGPUSharedMemory, &IADLXGPUMetricsSupport3::GetGPUSharedMemoryRange, GPU_CAP_SHARED_MEMORY, discoveredMetrics.sharedMemory);
	discoverV3("GPU Fan Duty", &IADLXGPUMetricsSupport3::IsSupportedGPUFanDuty, &IADLXGPUMetricsSupport3::GetGPUFanDutyRange, GPU_CAP_FAN_DUTY, discoveredMetrics.fanDuty);
...
template <typename T, typename MetricFn> int ADLXGpuTelemetry::ReadMetricV1(const char *name, MetricFn metricFn, IADLXGPUMetrics *gpuMetrics)
{
...
template <typename T, typename MetricFn> int ADLXGpuTelemetry::ReadMetricV3(const char *name, MetricFn metricFn, IADLXGPUMetrics *gpuMetrics)
{
...
GpuMetricsSnapshot ADLXGpuTelemetry::Query()
{
	...
	auto randDouble = [&](double min, double max) { return std::uniform_real_distribution<double>(min, max)(rng); };

	auto randInt = [&](int min, int max) { return std::uniform_int_distribution<int>(min, max)(rng); };
	...
	if (ADLX_SUCCEEDED(res))
	{
		...
		// GPU clocks
		readInt("GPUClockSpeed", GPU_CAP_CLOCK, snapshot.clockSpeed, &IADLXGPUMetrics::GPUClockSpeed);
		readInt("GPUVRAMClockSpeed", GPU_CAP_VRAM_CLOCK, snapshot.vramClockSpeed, &IADLXGPUMetrics::GPUVRAMClockSpeed);

		// Temperatures
		readDouble("GPUTemperature", GPU_CAP_TEMP, snapshot.temperature, &IADLXGPUMetrics::GPUTemperature);
		readDouble("GPUHotspotTemperature", GPU_CAP_HOTSPOT, snapshot.hotspot, &IADLXGPUMetrics::GPUHotspotTemperature);

		// Memory temperature
		readDoubleV1("GPUMemoryTemperature", GPU_CAP_MEM_TEMP, snapshot.memoryTemperature, &IADLXGPUMetrics1::GPUMemoryTemperature);

		// Intake temperature
		readDouble("GPUIntakeTemperature", GPU_CAP_INTAKE_TEMP, snapshot.intakeTemperature, &IADLXGPUMetrics::GPUIntakeTemperature);

		// Power
		readDouble("GPUPower", GPU_CAP_POWER, snapshot.power, &IADLXGPUMetrics::GPUPower);
		readDouble("GPUTotalBoardPower", GPU_CAP_BOARD_POWER, snapshot.totalBoardPower, &IADLXGPUMetrics::GPUTotalBoardPower);

		// Fan
		readInt("GPUFanSpeed", GPU_CAP_FAN_SPEED, snapshot.fanSpeed, &IADLXGPUMetrics::GPUFanSpeed);
		readIntV3("GPUFanDuty", GPU_CAP_FAN_DUTY, snapshot.fanDuty, &IADLXGPUMetrics3::GPUFanDuty);

		// Memory
		readInt("GPUVRAM", GPU_CAP_VRAM_USAGE, snapshot.vram, &IADLXGPUMetrics::GPUVRAM);
		readIntV3("GPUSharedMemory", GPU_CAP_SHARED_MEMORY, snapshot.sharedMemory, &IADLXGPUMetrics3::GPUSharedMemory);

		// Voltage
		readInt("GPUVoltage", GPU_CAP_VOLTAGE, snapshot.voltage, &IADLXGPUMetrics::GPUVoltage);

		// NPU
		readIntV1("NPUFrequency", GPU_CAP_NPU_FREQ, snapshot.npuFrequency, &IADLXGPUMetrics1::NPUFrequency);
		readIntV1("NPUActivityLevel", GPU_CAP_NPU_ACTIVITY, snapshot.npuActivityLevel, &IADLXGPUMetrics1::NPUActivityLevel);
...
void ADLXGpuTelemetry::PopulateGPUInfo(IADLXGPU *gpu)
{
	...
	// Extended VRAM information - ADLX GPU3
	IADLXGPU3Ptr gpu3(gpu);

	if (gpu3)
	{
		const char *microArchitecture = nullptr;
		if (gpu3->MicroArchitecture(&microArchitecture) == ADLX_OK)
			gpuInfo.SetMicroArchitecture(microArchitecture);

		adlx_uint data = 0;
		if (gpu3->HighestVRAMBandwidth(&data) == ADLX_OK)
			gpuInfo.SetHighestVRAMBandwidth(data);

		data = 0;
		if (gpu3->InvisibleVRAM(&data) == ADLX_OK)
			gpuInfo.SetInvisibleVRAM(data);

		data = 0;
		if (gpu3->VisibleVRAM(&data) == ADLX_OK)
			gpuInfo.SetVisibleVRAM(data);

		data = 0;
		if (gpu3->VRAMVendorRevId(&data) == ADLX_OK)
			gpuInfo.SetVRAMVendorRevId(data);

		data = 0;
		if (gpu3->VRAMBandwidth(&data) == ADLX_OK)
			gpuInfo.SetVRAMBandwidth(data);

		data = 0;
		if (gpu3->VRAMBitRate(&data) == ADLX_OK)
			gpuInfo.SetVRAMBitRate(data);
	}
	...
	if (gpu2)
	{
		// AMD Software / Driver information

		const char *driverInfo = nullptr;
		if (gpu2->AMDSoftwareEdition(&driverInfo) == ADLX_OK)
			gpuInfo.SetAMDSoftwareEdition(driverInfo);

		driverInfo = nullptr;
		if (gpu2->AMDSoftwareVersion(&driverInfo) == ADLX_OK)
			gpuInfo.SetAMDSoftwareVersion(driverInfo);

		driverInfo = nullptr;
		if (gpu2->DriverVersion(&driverInfo) == ADLX_OK)
			gpuInfo.SetDriverVersion(driverInfo);

		driverInfo = nullptr;
		if (gpu2->AMDWindowsDriverVersion(&driverInfo) == ADLX_OK)
			gpuInfo.SetAMDWindowsDriverVersion(driverInfo);

		adlx_uint year = 0, month = 0, day = 0;
		if (gpu2->AMDSoftwareReleaseDate(&year, &month, &day) == ADLX_OK)
			gpuInfo.SetAMDSoftwareReleaseDate(year, month, day);

		ADLX_LUID luid = {};
		if (gpu2->LUID(&luid) == ADLX_OK)
			gpuInfo.SetLuid(luid.lowPart, luid.highPart);

		// Applications running on this GPU
		// adlx_bool supported = false;
		// if (gpu2->IsSupportedApplicationList(&supported) == ADLX_OK)
		// 	gpuInfo.applicationListSupported = supported;

		// if (supported)
		// {
		// 	IADLXApplicationListPtr appList;

		// 	if (gpu2->GetApplications(&appList) == ADLX_OK && appList)
		// 	{
		// 		for (auto it = appList->Begin(); it != appList->End(); ++it)
		// 		{
		// 			IADLXApplicationPtr app;

		// 			if (appList->At(it, &app) != ADLX_OK || !app)
		// 				continue;
		...
		// 			app->GPUDependencyType(&dependency);

		// 			gpuInfo.AddGpuApplication(pid, name, fullPath, dependency);
		// 		}
		// 	}
		// }
	}
...
void ADLXGpuTelemetry::Probe()
{
	...
	// GPU usage
	probeDouble("GPUUsage", GPU_CAP_USAGE, m_snapshot.usage, &IADLXGPUMetrics::GPUUsage);

	// GPU clocks
	probeInt("GPUClockSpeed", GPU_CAP_CLOCK, m_snapshot.clockSpeed, &IADLXGPUMetrics::GPUClockSpeed);
	probeInt("GPUVRAMClockSpeed", GPU_CAP_VRAM_CLOCK, m_snapshot.vramClockSpeed, &IADLXGPUMetrics::GPUVRAMClockSpeed);

	// Temperatures
	probeDouble("GPUTemperature", GPU_CAP_TEMP, m_snapshot.temperature, &IADLXGPUMetrics::GPUTemperature);
	probeDouble("GPUHotspotTemperature", GPU_CAP_HOTSPOT, m_snapshot.hotspot, &IADLXGPUMetrics::GPUHotspotTemperature);
	probeDoubleV1("GPUMemoryTemperature", GPU_CAP_MEM_TEMP, m_snapshot.memoryTemperature, &IADLXGPUMetrics1::GPUMemoryTemperature);
	probeDouble("GPUIntakeTemperature", GPU_CAP_INTAKE_TEMP, m_snapshot.intakeTemperature, &IADLXGPUMetrics::GPUIntakeTemperature);

	// Power
	probeDouble("GPUPower", GPU_CAP_POWER, m_snapshot.power, &IADLXGPUMetrics::GPUPower);
	probeDouble("GPUTotalBoardPower", GPU_CAP_BOARD_POWER, m_snapshot.totalBoardPower, &IADLXGPUMetrics::GPUTotalBoardPower);

	// Fan
	probeInt("GPUFanSpeed", GPU_CAP_FAN_SPEED, m_snapshot.fanSpeed, &IADLXGPUMetrics::GPUFanSpeed);
	probeIntV3("GPUFanDuty", GPU_CAP_FAN_DUTY, m_snapshot.fanDuty, &IADLXGPUMetrics3::GPUFanDuty);

	// Memory
	probeInt("GPUVRAM", GPU_CAP_VRAM_USAGE, m_snapshot.vram, &IADLXGPUMetrics::GPUVRAM);
	probeIntV3("GPUSharedMemory", GPU_CAP_SHARED_MEMORY, m_snapshot.sharedMemory, &IADLXGPUMetrics3::GPUSharedMemory);

	// Voltage
	probeInt("GPUVoltage", GPU_CAP_VOLTAGE, m_snapshot.voltage, &IADLXGPUMetrics::GPUVoltage);

	// NPU
	probeIntV1("NPUFrequency", GPU_CAP_NPU_FREQ, m_snapshot.npuFrequency, &IADLXGPUMetrics1::NPUFrequency);
	probeIntV1("NPUActivityLevel", GPU_CAP_NPU_ACTIVITY, m_snapshot.npuActivityLevel, &IADLXGPUMetrics1::NPUActivityLevel);
