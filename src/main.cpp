...
void PaintGamepadStatus(HDC hdc)
{
	...
	// C++ short-circuit evaluation: left operand is evaluated first
	// If g_gamepadType is nullptr, the right operand is not evaluated
	const bool isVisible = g_gamepad && g_gamepad->GetTransport() == GamePad::Transport::Bluetooth;

	// Status is no longer visible (no gamepad, USB, etc.).
	if (!isVisible)
	{
	...
	const int batteryLevel = g_gamepad->BatteryLevel();
	const bool isCharging = g_gamepad->IsCharging();
	...
	wchar_t buffer[10];
	swprintf_s(buffer, L"%d%%%ls", batteryLevel, isCharging ? L"⚡" : L" ");
...
{
	static GamePad::Transport lastTransport = GamePad::Transport::None;

	const auto transport = g_gamepad ? g_gamepad->GetTransport() : GamePad::Transport::None;

	const int dotDiameter = MulDiv(6, g_fontSize, FONTSIZE);
	const int dotGap = MulDiv(3, g_fontSize, FONTSIZE);

	// If the previous state had a transport dot, clear its area.
	if (lastTransport != GamePad::Transport::None)
	{
...
{
	static bool lastGamePadEnabled = false;
	static GamePad::Transport lastTransport = GamePad::Transport::None;
	static bool initialized = false;
	const auto currentTransport = g_gamepad ? g_gamepad->GetTransport() : GamePad::Transport::None;
	const bool isGamePadEnabled = g_gamepadType != GamePad::Type::None;

	if (!g_forceFrameRedraw)
		if (initialized && lastGamePadEnabled == isGamePadEnabled && lastTransport == currentTransport)
			return;

	initialized = true;
	lastGamePadEnabled = isGamePadEnabled;
...
LayoutMetrics CalculateLayoutMetrics(HDC hdc)
{
	...
	// Window
	// ------------------------------------------------------------
	m.windowWidth = m.border * 2 + m.paddingSide * 2 + m.labelWidth + m.gap + m.valueWidth;

	m.windowHeight = m.titlePadding + m.titleHeight + m.titlePadding + m.paddingTop + m.lineHeight * g_lineCount + m.lineGap * g_lineCount + m.separatorHeight * g_lineCount + m.lineGap * g_lineCount + m.spacer + m.lineHeight2 + m.lineGap + m.lineHeight2 + m.lineGap + m.cardHeight + m.paddingBottom + m.border;
...
void DrawTag(HDC hdc, int x, int y, LPCWSTR text, int topPadding, int sidePadding, COLORREF color)
{
	...
	RECT tagRc = {x - sidePadding, y - topPadding, x + g_layoutMetrics.tagCharWidth * textLen + sidePadding, y + g_layoutMetrics.tagCharHeight + topPadding};
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
		...
		default:
		{
			...
			if (LOWORD(wParam) >= IDM_WEBSERVER_BASE && LOWORD(wParam) < IDM_WEBSERVER_BASE + list.size())
			{
				...
				if (!g_webServer.LaunchServerOnInterface(netIf))
				{
					...
					std::wstring message = L"Failed to launch the web server on interface\n\n"
										   L"Interface: " +
										   netIf.adapterName +
										   L"\n"
										   L"Address: " +
				...
				{
					std::wstring message = L"The web server has started successfully.\n\n"
										   L"URL:\n"
										   L"https://" +
										   netIf.address + L":" + WEBSERVER_PORT +
										   L"\n\n"
			...
			else if (LOWORD(wParam) >= IDM_ENABLEGAMEPAD_BASE || LOWORD(wParam) <= (IDM_ENABLEGAMEPAD_BASE + 3))
			{
				...
				{
				case 0:
					LOG_DEBUG("[App] Gamepad Screenshot Capture OFF");
					SelectGamePad(GamePad::Type::None, hwnd);
					break;
	...
	case WM_APP_SCREENSHOT_KEY_ERROR:
	{
		DWORD error = static_cast<DWORD>(wParam);

		if (error == ERROR_HOTKEY_ALREADY_REGISTERED)
		{
			LOG_ERROR("[App] Failed to register key: hotkey already taken (%d)", ERROR_HOTKEY_ALREADY_REGISTERED);
			TaskDialog(nullptr, nullptr, L"Key Error", L"Failed to register key.", L"\"Scroll Lock\" is already taken, set a new hotkey.", TDCBF_OK_BUTTON, TD_ERROR_ICON, nullptr);
		}
		else
		{
			LOG_ERROR("[App] Failed to register key: error %d", error);

			std::wstring message = L"Error " + std::to_wstring(error) + L": " + GetLastErrorMessage(error) + L"\nScreenshot is disabled.";
			TaskDialog(nullptr, nullptr, L"Key Error", L"Failed to register key.", message.c_str(), TDCBF_OK_BUTTON, TD_ERROR_ICON, nullptr);
		}
...
int WINAPI wWinMain(HINSTANCE hInstance, HINSTANCE, PWSTR, [[maybe_unused]] int nCmdShow)
{
	...
	// Register Scroll Lock as a global hotkey
	if (!RegisterHotKey(nullptr, HOTKEY_SCREENSHOT, 0, g_screenshotKey))
		PostMessage(hwnd, WM_APP_SCREENSHOT_KEY_ERROR, static_cast<WPARAM>(GetLastError()), 0);
	else
	...
	// PresentMon
	{
		...
		case -1:
		{
			TaskDialog(nullptr, nullptr, L"PresentMon SDK Not Found", L"Unable to load the PresentMon SDK.", L"The PresentMon API DLL could not be found or loaded. Some FPS monitoring features will be unavailable.", TDCBF_OK_BUTTON, TD_WARNING_ICON, nullptr);
			break;
		}
		case -2:
		{
			TaskDialog(nullptr, nullptr, L"PresentMon Service Not Running", L"Unable to connect to the PresentMon service.", L"The PresentMon service is not running. Some FPS monitoring features will be unavailable.", TDCBF_OK_BUTTON, TD_WARNING_ICON, nullptr);
			break;
		}
		case -3:
		{
			TaskDialog(nullptr, nullptr, L"Unable to Open Session", L"The PresentMon session could not be opened.", L"Some FPS monitoring features will be unavailable.", TDCBF_OK_BUTTON, TD_WARNING_ICON, nullptr);
			break;
		}

		case -4:
		{
			TaskDialog(nullptr, nullptr, L"PresentMon API Unavailable", L"The PresentMon API could not be initialized.", L"The required PresentMon API function could not be found in the DLL. Some FPS monitoring features will be unavailable.", TDCBF_OK_BUTTON, TD_WARNING_ICON, nullptr);
			break;
		}
		default:
			break;
