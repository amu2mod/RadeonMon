...
static bool IsVirtualAdapter(const IP_ADAPTER_ADDRESSES *adapter)
{
	...
	static const wchar_t *virtualNames[] = {L"Hyper-V", L"VMware", L"VirtualBox", L"VBox", L"Virtual", L"Docker", L"WSL", L"VPN", L"TAP", L"WireGuard", L"ZeroTier", L"Loopback"};
...
bool NetworkManager::Initialize(HWND hwnd)
{
	...
	DWORD result = NotifyIpInterfaceChange(AF_UNSPEC, &NetworkManager::InterfaceChanged, this,
...
std::vector<NetworkInterface> NetworkManager::Discover() const
{
	...
	for (auto adapter = adapters; adapter; adapter = adapter->Next)
	{
		...
		for (auto unicast = adapter->FirstUnicastAddress; unicast; unicast = unicast->Next)
		{
			...
			auto it = std::find_if(addresses.begin(), addresses.end(), [&](const NetworkInterface &item) { return item.address == address; });
